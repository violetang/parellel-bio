 ...
 *
 * Functions to initialize the model.  The only public function is
 * initialize_model().  Initialization is done by randomly assigning
...
int randem_initialize_mixture(options *opt, data *dat, model *mod)
{
	...
	for (i = 0; i < n_init; i++) {
		...
		if (logL > max_logL) {
			max_logL = logL;
			COPY_1ARRAY(I_K, mod->I_K, dat->I);
		}
	}
	COPY_1ARRAY(mod->I_K, I_K, dat->I);
...
int randem_initialize_admixture(options *opt, data *dat, model *mod)
{
	...
	for (i = 0; i < n_init; i++) {
		...
		if (logL > max_logL) {
			max_logL = logL;
			COPY_2ARRAY(IL_K_max, mod->IL_K, dat->L);
		}
	}
	COPY_2ARRAY(mod->IL_K, IL_K_max, dat->L);
			...
			if (dat->L_alleles) {
				...
				for (m = m_start; m < dat->uniquealleles[l]; m++)
					if (dat->IL[i][l] == dat->L_alleles[l][m])
						mod->pKLM[mod->IL_K[i][l]][l][m]++;
			} else {
				mod->pKLM[mod->IL_K[i][l]][l][dat->IL[i][l]]++;
			}
		}
